...
public class ChallengeTwo extends Challenges {
	...
	public ChallengeTwo(int x, int y, int z) {
		...
		player.setPosition(x-(runwayWidth/2)-2,y+3	,z-(runwaySize/2));
			...
			for(int y =0; y<blockMapping[0].length; y++){
				for(int z =0; z<blockMapping[0][0].length; z++){
					if(blockMapping[x][y][z]!=null && blockMapping[x][y][z]!=worldIn.getBlockState(new BlockPos( this.x-(runwayWidth/2)+1+x, this.y-2+y, z-(runwaySize/4)-score+2+z))){
						//Minecraft.getMinecraft().thePlayer.sendChatMessage("You may not place any blocks in the challenge area.");
	...
	public boolean run(){
		...
		displayTime.setScorePoints((int) (120-((System.currentTimeMillis()-startTime)/1000)));
		int increase = 0;
		...
		showScore();
		//Minecraft.getMinecraft().thePlayer.sendChatMessage("Time left = "+(120-((System.currentTimeMillis()-startTime)/1000)));
		waitTime--;
		int score = getScore();
		switch(getScore()){
